...
iterations=5000;
min_Cm_Dm_all = zeros(3, iterations);
...
for it = 1:iterations
    ...
    for s = 1:3
        for i = 1:num_cells
            ...
            for j= 1 : num_cells 
                interference = 0 ;
                if i~=j
                    interference = Beam_power_random{s, j}*Channel_gain{s,j} ;
                end
    
                sum_interference = sum_interference + interference;
            end
    
            Beam_SINR_random{s, i} = (Beam_power_random{s, i}*Channel_gain{s,i}) / ((kb*Trx*B)+sum_interference) ;  % 1/n씩
    ...
    % Demand of traffic m-th beam 
    
    Demand_cell = cell(3, num_cells); 
    for s = 1:3
        for i = 1:num_cells
    
            Demand_cell{s, i} = User_cell{s, i}*5000;  
    ...
    for s = 1:3
        for i = 1:num_cells
            % Beam_capacity와 Demand_cell의 비율을 계산하여 temp에 저장
            temp{s, i} = Beam_capacity_random{s, i} / Demand_cell{s, i};      
        end
    end
    ...
    for s = 1:3
        % temp의 s번째 행에서 모든 열에 대해 최소값을 계산
        min_Cm_Dm_random{s} = min_exclude_zero(cell2mat(temp(s,:)));
    end


    % 결과 저장
    min_Cm_Dm_all(:, it) = cell2mat(min_Cm_Dm);
    min_Cm_Dm_random_all(:, it) = cell2mat(min_Cm_Dm_random);

end


min_Cm_Dm_avg = mean(min_Cm_Dm_all, 2);
min_Cm_Dm_random_avg = mean(min_Cm_Dm_random_all, 2);

% 결과 출력
fprintf('Uniform Allocation 평균 min_Cm_Dm:\n');
disp(min_Cm_Dm_avg);
